...
if( ! function_exists("rtframework_get_settings") ){
	...
	function rtframework_get_settings( $function_call = false ){
		...
		if( is_singular() || $function_call || $wp_customize || $woocommerce_start_page ){
			...
			// no need to compare with the theme 
...
if( ! function_exists("rtframework_body_class_name") ){
	...
	function rtframework_body_class_name($classes) { 
		...
		$classes[] = rtframework_get_setting( "naturalife_sticky_mobile_header" ) ? 'sticky-mobile-header' : "";
  		...
  		//Polylang
		$classes[] = class_exists("Polylang") ? 'polylang' : "";
...
if( ! function_exists("rtframework_custom_oembed_filter") ){
	 ...
	 * @return html
	 */
	function rtframework_custom_oembed_filter($return, $data, $url) {	
...
add_filter( 'oembed_dataparse', 'rtframework_custom_oembed_filter', 10, 3 ) ;
...
if( ! function_exists("rtframework_mobile_logo_function") ){
	...
	function rtframework_mobile_logo_function(){
			...
			$mobile_header_height = max(intval(rtframework_get_setting("mobile_header_height")), 30);


			if( ! empty( $mobile_logo ) ){

					//retina srcsets
					if( ! empty( $retina_mobile_logo ) ){
						$logo_srcset = ' srcset="'.$retina_mobile_logo[0].' 1.3x"';  

						//max-height
						$max_height_class = $retina_mobile_logo[2] / 2 > $mobile_header_height ? " logo-auto-height" : "";
					}else{
						$logo_srcset = '';   

						//max-height
						$max_height_class = $mobile_logo[2] > $mobile_header_height ? " logo-auto-height" : "";
					}


					//logo outputs
					$logo_output =  ! empty( $mobile_logo ) ? sprintf( '<img width="%1$s" height="%2$s" src="%3$s" alt="%4$s" class="mobile-logo%5$s"%6$s />', esc_attr( $mobile_logo[1] ), esc_attr( $mobile_logo[2] ), esc_url($mobile_logo[0]), esc_html(get_bloginfo('name')), $max_height_class, $logo_srcset ) : "" ;
...
if( ! function_exists("rtframework_get_post_navigation") ){
	...
	function rtframework_get_post_navigation(){
		...
		if ( is_singular( "post" ) ){
			...
			$start_page = rtframework_wpml_translated_page_id( rtframework_get_setting( 'rt_blogpage' ) );
		}
		...
		if ( is_singular( "portfolio" ) ){
			...
			$start_page = rtframework_wpml_translated_page_id( rtframework_get_setting( 'rt_portfoliopage' ) );
		}
...
if( ! function_exists("rtframework_portfolio_comments") ){
	 ...
	 * @return HTML
	 */
	function rtframework_portfolio_comments() {
		if( 
			! get_theme_mod( 'naturalife_portfolio_comments' ) ||
...
add_action( 'rtframework_end_main_content', "rtframework_portfolio_comments",10); 
...
if( ! function_exists("rtframework_side_panel_languages") ){
	...
	function rtframework_side_panel_languages(){
		...
		$default_locale = isset( $current_language['default_locale'] ) ? $current_language['default_locale'] : $code;
		?>
			...
			<ul class="menu">
				<li class="menu-item-has-children">
					<a href="#" title="<?php esc_attr_e("Switch the language","naturalife");?>">
						...
						<span class="rt-language-name" lang="<?php echo esc_attr( $default_locale );?>"><?php echo esc_attr($current_language['native_name'])?></span>
					</a>
